...
•	In this directory run the following commands (in the command prompt):
   o	truffle init
   o	npm init -y
   o	npm  install -E openzeppelin-solidity
   o	npm install truffle-assertions
   o	npm install -g ganache-cli
...
o	First migrate the contracts with the command: truffle migrate --compile-all –reset
o	Then open truffle console with the command: truffle console --network ganache
